...
class EpisodesCheckerStorageTest {
    @Test
    fun putEpisode_thenPutAllWithStaleData_doesNotDegradeState() =
        runTest {
    ...
    @Test
    fun putAll_resolverHonorsWatchedPrioritySeekDeltaAndLastAccessFallback() =
        runTest {
            val storage = createStorage()
    ...
    @Test
    fun concurrentPutPutAllRemove_doesNotCrash_andStateRemainsConsistent() =
        runTest {
            val storage = createStorage()
    ...
    @Test
    fun saveAll_applyRunsOnBackgroundThread_notOnCallerThread() =
        runTest {
    ...
    @Test
    fun putAllEpisodeBatched_savesInBatches_notPerItem() =
        runTest {
            val prefs = EpisodesInMemorySharedPreferences()
            val storage = createStorage(sharedPreferences = prefs)
    ...
    @Test
    fun rapidPutEpisode_coalescesSingleItemWrites() =
        runTest {
            val prefs = EpisodesInMemorySharedPreferences()
            val storage = createStorage(sharedPreferences = prefs)
    ...
    private fun createStorage(sharedPreferences: SharedPreferences = EpisodesInMemorySharedPreferences()): EpisodesCheckerStorage {
    ...
    private suspend fun waitUntil(predicate: () -> Boolean) {
        withContext(Dispatchers.Default) {
            repeat(100) {
                if (predicate()) return@withContext
                delay(25)
            }
