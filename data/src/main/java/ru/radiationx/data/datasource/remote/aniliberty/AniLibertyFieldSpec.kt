...
data class AniLibertyFields(
    ...
    override fun includeParam(): String? = include.takeIf { it.isNotEmpty() }?.joinToString(",") { it.value }

    override fun excludeParam(): String? = exclude.takeIf { it.isNotEmpty() }?.joinToString(",") { it.value }
