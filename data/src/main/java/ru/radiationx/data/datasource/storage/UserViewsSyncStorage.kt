...
class UserViewsSyncStorage @Inject constructor(
    ...
    override suspend fun upsertPendingUpload(upload: UserViewPendingUpload) {
        val updated = pendingUploadsRelay.getValue()
    ...
    override suspend fun removePendingUploadsByEpisodeIds(episodeIds: Collection<EpisodeId>) {
        ...
        val episodeIdSet = episodeIds.mapTo(mutableSetOf()) { episodeId ->
    ...
    private suspend fun loadPendingUploads(): List<UserViewPendingUpload> = withContext(Dispatchers.IO) {
        sharedPreferences.getString(KEY_PENDING_UPLOADS, null)
            ?.let { json -> pendingUploadsAdapter.fromJson(json) }
            .orEmpty()
    }

    private suspend fun savePendingUploads(value: List<UserViewPendingUpload>) = withContext(Dispatchers.IO) {
        sharedPreferences.edit {
            if (value.isEmpty()) {
