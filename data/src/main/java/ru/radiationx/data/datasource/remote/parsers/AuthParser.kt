...
class AuthParser @Inject constructor() {
    ...
    fun checkOtpError(error: Throwable): Throwable = when (error) {
    ...
    private fun mapOtpApiError(error: ApiError): Throwable = when (error.description) {
    ...
    private fun parseOtpErrorBody(bodyText: String): ParsedOtpError {
        ...
        return runCatching {
            ...
            val directDescription = json.optString("description").takeIf { it.isNotBlank() }
            ...
            val nestedDescription = nestedErrorObj?.optString("description")?.takeIf { it.isNotBlank() }
                ?: nestedErrorObj?.optString("error")?.takeIf { it.isNotBlank() }
                ?: nestedErrorObj?.optString("key")?.takeIf { it.isNotBlank() }

            val nestedMessage = nestedErrorObj?.optString("message")?.takeIf { it.isNotBlank() }
