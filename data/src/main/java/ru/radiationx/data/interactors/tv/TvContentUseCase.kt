...
class TvContentUseCaseImpl
    ...
    constructor(
        ...
        private val scheduleFieldsForCards: AniLibertyReleaseFields =
        ...
        override suspend fun loadMainSchedule(currentTimeMs: Long): MainSchedulePayload {
            ...
            val title =
                if (Date(currentTimeMs).isSameDay(Date(mskTime))) {
                    "Ожидается сегодня"
                } else {
        ...
        private fun publishDayToCalendarDayOrNull(day: Int): Int? =
            when (day) {
                1 -> Calendar.MONDAY
                2 -> Calendar.TUESDAY
        ...
        private fun calendarDayToAniLibertyPublishDay(calendarDay: Int): Int? =
            when (calendarDay) {
                Calendar.MONDAY -> 1
                Calendar.TUESDAY -> 2
