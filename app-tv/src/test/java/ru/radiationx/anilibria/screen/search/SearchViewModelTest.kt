...
class SearchViewModelTest {
    ...
    @Test
    fun completedOnly_filtersPrimaryCatalogResultsByResolvedReleaseStatus() =
        runTest {
            val completedRelease =
                fakeRelease(
                    id = 1,
                    title = "completed-release",
                    statusCode = Release.STATUS_CODE_COMPLETE,
                )
            val notOngoingRelease =
                fakeRelease(
                    id = 2,
            ...
            coEvery { tvSearchUseCase.searchReleases(any(), any()) } returns
                listOf(
                    completedRelease,
                    notOngoingRelease,
                )
    ...
    @Test
    fun completedOnly_filtersRepositoryFallbackResultsByResolvedReleaseStatus() =
        runTest {
            val tvSearchUseCase = mockk<TvSearchUseCase>()
            coEvery { tvSearchUseCase.searchReleases(any(), any()) } returns
                listOf(
            ...
            val searchRepository = mockk<SearchRepository>()
            coEvery { searchRepository.searchReleases(any(), any()) } returns
                paginatedResponse(
                    listOf(
                        completedFallback,
                        fakeRelease(
    ...
    @Test
    fun dateSort_prioritizesReleaseYearOverTorrentFreshness() =
        runTest {
            val tvSearchUseCase = mockk<TvSearchUseCase>()
            coEvery { tvSearchUseCase.searchReleases(any(), any()) } returns
                listOf(
    ...
    @Test
    fun sameForm_isNotReloadedTwice() =
        runTest {
            val tvSearchUseCase = mockk<TvSearchUseCase>()
            coEvery { tvSearchUseCase.searchReleases(any(), any()) } returns
                listOf(
    ...
    private fun paginatedResponse(data: List<Release>): Paginated<Release> =
        Paginated(
            data = data,
            page = 1,
            allPages = 1,
